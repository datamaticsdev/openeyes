...
<aside class="column sidebar episodes-and-events">
    ...
    <?php if ((!empty($ordered_episodes) || !empty($legacyepisodes) || !empty($supportserviceepisodes)) && ($this->checkAccess('OprnCreateEpisode')) && $this->firm->getSubspecialtyId() != $current_episode->firm->getSubspecialtyId()) { ?>
            ...
            <div class="panel specialty">
                ...
                <?php foreach ($specialty_episodes['episodes'] as $i => $episode) { ?>
                    ...
                    <section class="panel episode<?php echo empty($episode->end_date) ? ' closed' : ' open' ?>">
                        ...
                        <a href="#"
                           class="toggle-trigger toggle-<?php if ((!$current_episode || $current_episode->id != $episode->id) && $episode->hidden) { ?>show<?php } else { ?>hide<?php } ?>">
                        ...
                        <ol class="events-overview"
                            <?php if ($episode->hidden) { ?>style="display:block" <?php } else { ?> style="display : none"<?php } ?>>
                            <?php
                            foreach ($episode->events as $event) {
                                ...
                                <li>
                                    <a href="<?php echo $event_path . $event->id ?>" data-id="<?php echo $event->id ?>">
                                        ...
                                        ?>
                                        <img src="<?php echo $assetpath . 'img/small.png' ?>" alt="op" width="19"
                                             height="19"/>
                                    </a>
                                </li>
                            <?php } ?>
                        ...
                        <!-- Episode events -->
                        <div <?php if ($episode->hidden) { ?>class="events-container hide"
                             <?php } else { ?>class="events-container show"<?php } ?>>
                            ...
                            if ($this->checkAccess('OprnCreateEvent')) {
                                ...
                                <button
                                    ...
                                    <?php if (!$enabled) {
                                        echo 'title="Please switch firm to add an event to this episode"';
                                    } ?>>
                                    <span class="icon-button-small-plus-sign"></span>
                                    Add event
                                </button>
                            ...
                            $subspecialty_data = $ssa ? array_intersect_key($ssa->subspecialty->attributes,
                                array_flip(array('id', 'name'))) : array();
                            if ($enabled) { ?>
                                <script type="text/html" id="add-new-event-template"
                                        data-specialty='<?php echo json_encode($subspecialty_data); ?>'>
                                        ...
                                        <a href="<?php echo $event_path . $event->id ?>"
                                           data-id="<?php echo $event->id ?>">
											<span class="event-type<?php if ($event->hasIssue()) { ?> alert<?php } ?>">
                                                ...
                                                ?>
                                                <img src="<?php echo $assetpath . 'img/' . $imgName ?>" alt="op"
                                                     width="19" height="19"/>
											</span>
											<span
                    ...
                    <div class="episode_details hide" id="episode-details-<?php echo $episode->id ?>">
                        <div class="row"><span
                                class="label">Start date:</span><?php echo $episode->NHSDate('start_date'); ?></div>
                        <div class="row"><span
                                class="label">End date:</span><?php echo $episode->end_date ? $episode->NHSDate('end_date') : '-' ?>
                        </div>
                        <div class="row"><span
                                class="label">Principal eye:</span><?php echo ($episode->diagnosis) ? ($episode->eye ? $episode->eye->name : 'None') : 'No diagnosis' ?>
                        </div>
                        <div class="row"><span
                                class="label">Principal diagnosis:</span><?php echo ($episode->diagnosis) ? ($episode->diagnosis ? $episode->diagnosis->term : 'none') : 'No diagnosis' ?>
                        </div>
                        <div class="row"><span
                                class="label">Subspecialty:</span><?php echo CHtml::encode($episode->getSubspecialtyText()) ?>
                        </div>
                        <div class="row"><span
                                class="label">Consultant firm:</span><?php echo $episode->firm ? CHtml::encode($episode->firm->name) : 'N/A' ?>
                        </div>
                        <img class="folderIcon"
                             src="<?php echo Yii::app()->assetManager->createUrl('img/_elements/icons/folder_open.png') ?>"
                             alt="folder open"/>
                    </div>
                <?php } ?>
